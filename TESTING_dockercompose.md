...
docker run --rm --net drupalcontribution-org_default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=drupalcontribution-org | grep Up | grep drupalcontribution-org_nginx_1
docker ps --filter label=com.docker.compose.project=drupalcontribution-org | grep Up | grep drupalcontribution-org_mariadb_1
docker ps --filter label=com.docker.compose.project=drupalcontribution-org | grep Up | grep drupalcontribution-org_php_1
docker ps --filter label=com.docker.compose.project=drupalcontribution-org | grep Up | grep drupalcontribution-org_cli_1

# Should ssh against the cli container by default
docker-compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker-compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep drupalcontribution-org.docker.amazee.io
